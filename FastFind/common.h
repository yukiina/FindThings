...
#define __DB_PATH__ ".\\"
#define __DB_NAME__ "doc.db"
#define __TB_NAME__ "doc_tb"
...
// DirList: path 文件路径 输入   fileList, dirList: 文件路径中包含的文件和文件夹 输出
static void DirList(const string& pathStr, vector<string>& fileList, vector<string>& dirList) {
	path str(pathStr);
	if (!exists(str)) {		//必须先检测目录是否存在才能使用文件入口.
		ERROE_LOG("输入的path不存在");
		return;
	}
	try {
		directory_entry entry(str);
		if (entry.is_directory()) { // 指定文件是一个目录
			directory_iterator list(str);
			for (const auto& it : list) {     //遍历目录内容
				if (it.is_directory()) {
					dirList.push_back(it.path().filename().string()); //把所有文件夹加入容器
				}
				else {
					fileList.push_back(it.path().filename().string()); //把所有文件加入容器
				}
...
// 汉字转拼音首字母
static string ChineseConvertPinYinInitials(const std::string& name)
{
	//assert(name.c_str());
	// 仅生成拼音首字母内容
	...
	static const char* secondSecTable = "CJWGNSPGCGNE[Y[BTYYZDXYKYGT[JNNJQMBSGZSCYJSYY[PGKBZGY[YWJKGKLJYWKPJQHY[W[DZLSGMRYPYWWCCKZN\
...
// 高亮
static void ColorPrintf(const char* str)
{
